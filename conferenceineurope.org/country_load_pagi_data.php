			...
			<div class="up-comming-sec">
				<div class="row">
					...
					if (isset($_REQUEST['date'])) {
						$date_class = "hide";
						$date = $_REQUEST['date'];
						$sqlcntfrpgntn = "SELECT COUNT(*) 'total_rows' FROM `event` WHERE `country` like '%$country%' and `status`='Accept' and `event_stat` like '$date%' and  `event_stat` >= curdate()";
					} else
						$sqlcntfrpgntn = "SELECT COUNT(*) 'total_rows' FROM `event` WHERE `country` like '%$country%' and ( `status`='Accept' and `event_stat` > curdate() )";
					...
					if ($total_rows > 0) {
						...
						if (isset($_REQUEST['date'])) {
							$date = $_REQUEST['date'];
							$exe_sql_homecon_list = "SELECT * FROM `event` WHERE `country` like '%$country%' and `status`='Accept' and `event_stat` like '$date%' and  `event_stat` >= curdate() ORDER BY `event_stat` ";
						} else
							$exe_sql_homecon_list = "SELECT * FROM `event` WHERE `country` like '%$country%' and ( `status`='Accept' and `event_stat` > curdate() ) ORDER BY `event_stat` ASC limit $offset, $limit";
					...
					?>
						...
						<div class="pull-right" <?php if (isset($date_class)) {
