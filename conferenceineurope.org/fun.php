...
function check_row_prom($id)
    {
	global $link;
	$cc=mysqli_num_rows(mysqli_query($link,"SELECT * FROM `promote` WHERE `event_id`='$id' and `exp_date`>= curdate() and `promote_type`= 'ALLCONFLINEEVENT'"));
    return($cc);
    }
...
return($str); 
	...
	function  get_event_cat_name($cat)
{
global $link;
$sql_ev=mysqli_fetch_array(mysqli_query($link,"SELECT * FROM `event_topic` WHERE `topic_id`='$cat'"));
return($sql_ev['topic']);		
}
